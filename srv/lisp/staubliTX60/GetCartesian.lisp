...
(defclass <GetCartesian-request> (ros-message)
  ()
)
(defmethod serialize ((msg <GetCartesian-request>) ostream)
  "Serializes a message object of type '<GetCartesian-request>"
)
(defmethod deserialize ((msg <GetCartesian-request>) istream)
...
(defmethod message-definition ((type (eql '<GetCartesian-request>)))
  "Returns full string definition for message of type '<GetCartesian-request>"
  (format nil "~%"))
(defmethod serialization-length ((msg <GetCartesian-request>))
  (+ 0
))
(defmethod ros-message-to-list ((msg <GetCartesian-request>))
  "Converts a ROS message object to a list"
  (list '<GetCartesian-request>
))
;//! \htmlinclude GetCartesian-response.msg.html

(defclass <GetCartesian-response> (ros-message)
  ((x
    :reader x-val
    :initarg :x
    :type float
...
(defmethod serialize ((msg <GetCartesian-response>) ostream)
...
(defmethod deserialize ((msg <GetCartesian-response>) istream)
...
(defmethod md5sum ((type (eql '<GetCartesian-response>)))
  ...
  "88d995302418aa13534aa14fc9aeed94")
(defmethod message-definition ((type (eql '<GetCartesian-response>)))
...
(defmethod serialization-length ((msg <GetCartesian-response>))
  (+ 0
     8
     8
     8
...
(defmethod ros-message-to-list ((msg <GetCartesian-response>))
  "Converts a ROS message object to a list"
  (list '<GetCartesian-response>
    (cons ':x (x-val msg))
    (cons ':y (y-val msg))
    (cons ':z (z-val msg))
    (cons ':rx (rx-val msg))
...
(defmethod service-request-type ((msg (eql 'GetCartesian)))
  '<GetCartesian-request>)
(defmethod service-response-type ((msg (eql 'GetCartesian)))
